...
//
//   DISABLE_WAVETABLES        works; frees 10,608 bytes on the voice card
...
// KZ MOD 2026-09-17: leaves the 16 PPG-style wavetables and WAVEQUENCE out of
// the voice card. They cost 10,608 bytes of PROGMEM -- a third of its flash --
// which is what funds the new oscillator work.
//
// Undefine it for a classic build that keeps them. Both configurations are
// compatible with the same controller: the enum positions never move, so a
// patch referencing a wavetable loads either way. It renders the wave on a
// classic card and a polyBLEP saw on a v2 card.
// Define KZ_CLASSIC_WAVETABLES on the command line for the classic build;
// scripts/build_firmware.py --target voicecard --variant classic does that.
#ifndef KZ_CLASSIC_WAVETABLES
#define DISABLE_WAVETABLES
#endif
