...
// -----------------------------------------------------------------------------
//
// KZ MOD: the PPG-style wave bank and its index, split out of the generated
// resources so they can be left out of a build.
//
// These are 10,608 bytes -- a third of the voice card's flash -- and they are
// only reachable from the two wavetable renderers, which address them directly
// rather than through waveform_table. Keeping them here, behind
// DISABLE_WAVETABLES, is what lets one source tree produce both a classic build
// with the wavetables and a v2 build that spends that flash on new oscillators.
//
// The data is byte-for-byte what the resource compiler emitted; regenerating
// resources will not touch it. See voicecard/resources/waveforms.py.
...
// Must come before the definitions: a namespace-scope `const` has internal
// linkage in C++ unless an extern declaration is already in scope.
